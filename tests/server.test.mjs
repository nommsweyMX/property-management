...
test('API fails closed rather than pretending to save to Airtable',()=>usingServer(async base=>{const res=await fetch(base+'/api/tickets');assert.equal(res.status,503);assert.equal((await res.json()).error,'LIVE_INTEGRATION_NOT_ENABLED');}));
test('unconfigured public write route is not exposed',()=>usingServer(async base=>assert.equal((await fetch(base+'/api/tickets',{method:'POST'})).status,405)));
